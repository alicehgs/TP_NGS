...
#### 1.1-Download the polymorphism dataset
...
# We want to select only deleterious polymorphisms. We assume that any polymorphism located inside a coding sequence is either deleterious or neutral, and therefore select all the exonic polymorphisms for our analysis.
...
# As there are lots of different populations and super-populations, we can make a loop to automate the filtering by population and the analysis. 
...
# vcf_coding_polymorphism splits a .vcf into 3 .vcf files that contains respectively Synonymous, Non-Synonymous and Stop (LoF) polymorphisms
# To classify a polymorphism, we have to identify the codon to which it belongs as well as the amino acid encoded by this codon. 
#       If the variant amino acid is the same than the reference amino acid, then the polymorphism is Synonymous.
#       If the variant codon is a stop codon, then the polymorphism is LoF.
#       In all the other cases, the polymorphism is Non-Synonymous. 
# The classification requires to cross the information given by the .vcf file, the .gtf file and the .fasta file.
...
# Run vcf_coding_polymorphism.py
vcf_coding_polymorphism.py -f Homo_sapiens.GRCh38.cds.all.fa -v Chr20_intersect.vcf -g Homo_sapiens.GRCh38.94.chr.gtf 

#### 2.2-Meta-analysis
# See vcf_meta_analysis.py
# vcf_meta_analysis.py reproduce the analysis of Sohail et al., Science (2017).
