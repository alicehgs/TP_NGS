...
# Input: alignment (.bam)
# Ouput: alignment with duplicates marked (.bam) # father.marked_dups.bam
java -jar ${PICARD} MarkDuplicates \
	REMOVE_DUPLICATES=FALSE \
	I=${FILE_NAME}.bam \
	O=${FILE_NAME}.marked_dups.bam \
	M=${FILE_NAME}.dup_metrics.txt
# Les duplicata sont conservés, mais seulement non pris en compte dans l'analyse
...
#####################################
# Permet de corriger le fait que dans le premier alignement, le taux d'erreur est plus important sur les extrémités des reads.
...
################################
# Comparaison des SNP trouvés a priori dans notre jeu de données avec des SNP déjà bien caractérisés chez l'homme.
...
java -jar ${GATK} -T HaplotypeCaller \
                  -R ${REF_GENOME} \
                  -I ${FILE_NAME}.exome_recal_reads.bam \
                  -o ${FILE_NAME}.g.vcf  \
...
java -jar ${GATK} -T GenotypeGVCFs \
                  -R ${REF_GENOME} \
                  -o ${FILE_NAME}.vcf \ --variant ${FILE_NAME}.g.vcf
